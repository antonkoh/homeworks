class Application
  ...
  def add_cars(train)
    ...
    if !car_type.nil?
      ...
      puts "Вместительность вагона будет 0." if max == 0
  ...
  def edit_car_capacity(car)
    ...
    when car.class == PassengerCar
      all_occupied_desc = 'нет свободных места'
      ...
      not_enough_free_desc = 'Нет столько свободных мест.'
      not_enough_occupied_desc = 'Нет столько пассажиров на местах.'
    ...
    when car.class == CargoCar      
      ...
      not_enough_free_desc = 'Нет столько свободного места.'
      not_enough_occupied_desc = 'Нет столько груза.'
    ...
    when '1'
      ...
      not_enough = not_enough_free_desc
      done = occupy_done
      compare = car.available
    when '2'
      question = how_much_free_desc
      not_enough = not_enough_occupied_desc
      done = free_done
    ...
    if (action == '1' && car.available > 0) || (action == '2' && car.occupied > 0) 
      puts
      print "#{question} "
      car.class == PassengerCar ? load = gets.strip.to_i : load = gets.strip.to_f
      if compare < load
        puts
        puts "Действие не выполнено. #{not_enough}"
      else
